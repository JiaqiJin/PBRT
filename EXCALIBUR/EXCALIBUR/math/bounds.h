...
public:
    ...
    Point2<T> lerp(const Point2f& t) const {
        return Point2<T>(Rendering::lerp(t.x, pMin.x, pMax.x),
            Rendering::lerp(t.y, pMin.y, pMax.y));
    }
...
public:
    ...
    Point3<T> lerp(const Point3f& t) const {
        return Point3<T>(Rendering::lerp(t.x, pMin.x, pMax.x),
            Rendering::lerp(t.y, pMin.y, pMax.y),
            Rendering::lerp(t.z, pMin.z, pMax.z));
    }
...
inline bool AABB3<T>::intersectP(const Ray& ray, const Vector3f& invDir,
    ...
    // gamma函数用于误差分析
    tMax *= 1 + 2 * Rendering::gamma(3);
    tyMax *= 1 + 2 * Rendering::gamma(3);
