...
只要指数位不变，ulp不变，暂且把ulp当成绝对误差，绝对误差不会变，我们定义相对误差为f，绝对误差比当前数值，f = ulp/(a+b)
当c=1时，a+b取到最小值，相对误差f取到最大值 2^-23，f = 2^-23，
其实因为间隔为误差的两倍所以最终算出的相对误差为 ε = 2^-24
由此可分析出绝对误差范围
...
要注意的一点是，对于任何数来说，乘以2或除以2都不会引入误差(假设没有overflow与underflow)
更一般的总结，乘以或除以2整数次幂都不会引入误差
...
= (((a⊕b)⊕c)⊕d)
⊂ ((((a+b)(1 ± ε))+c)(1± ε)+d)(1± ε)
= (a + b)(1± ε)^3 + c(1± ε)^2 + d(1± ε)
上述表达式中有高次幂项，这在以后的使用中不方便，我们用如下表达式替换高次幂项
    (1± ε)^n ≤ (1± (n + 1)ε)
上式可以继续化简
(a + b)(1± 4ε) + c(1± 3ε) + d(1± 2ε)
 = a + b + c + d + [±4ε(a + b) ± 3εc ± 2εd].
由此可见，a+b产生的误差最多，所以将相加顺序从小到大排列可以降低误差
为了进一步降低误差 我们可以交换适当使用括号
((a ⊕ b) ⊕ (c ⊕ d))
= a + b + c + d + 3ε|a + b| + 3ε|c + d|,
用(1± ε)n 替换 (1± (n + 1)ε)之后，使用是方便了很多，但范围太大了，于是又提出了
(1± ε)n ≤ 1+θn
其中 |θn| ≤ nε / (1-nε)
最终，我们用 1 + γn 取代 (1± ε)^n，其中 γn = nε / (1-nε)，gamma函数在header文件中实现。
接下来解释一下加法误差传递  ±δa为a的上下浮动范围，b同理
    (a ± δa) ⊕ (b ± δb)
= ((a ± δa) + (b ± δb))(1± γ1)
= a + b + [±δa ± δb ± (a + b)γ1 ± γ1 * δa ± γ1 * δb].
可以看出上式的误差范围 [±δa ± δb ± (a + b)γ1 ± γ1δa ± γ1δb]
以上已经介绍了误差管理的基础知识，即将介绍的是ray与shape的求交计算的误差管理
这部分内容在ray.h中进行介绍
 */
